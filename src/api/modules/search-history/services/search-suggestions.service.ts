...
import { normalizeForHistory } from '../../../shared/utils/search.normalizer';
...
interface HistorySuggestionResult {
  term: string;
  count: number;
  score: number;
}

interface CatalogSuggestionResult {
  term: string;
  count: number;
}
...
export async function filterSuggestions(
  ...
  try {
    ...
    const results = await SearchHistory.aggregate<HistorySuggestionResult>(pipeline);

    // Basic sanitization
    const historySuggestions = results.filter((s) => s.term && s.term.length >= minLength);
    ...
    // categories
    const categoryAgg: PipelineStage[] = [
    ...
    const [catResults, tagResults] = await Promise.all([
      JobOffer.aggregate<CatalogSuggestionResult>(categoryAgg),
      JobOffer.aggregate<CatalogSuggestionResult>(tagsAgg),
    ]);
    ...
    const catalogSuggestions: Suggestion[] = catalogRaw.map((c) => ({
      term: c.term,
      count: c.count || 0,
    ...
    const merged = Array.from(mergedMap.values())
      .filter((s) => s.term && s.term.length >= minLength)
      .sort((a, b) => {
 ...
 */
export async function getTopSuggestions(limit: number = 10): Promise<Suggestion[]> {
  try {
    const minLength = 3;

    const topSearches = await SearchHistory.aggregate<{ term: string; count: number }>([
    ...
    const filtered = results.filter((s) => s.term && s.term.length >= minLength);
